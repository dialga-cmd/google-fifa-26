...
def create_stadium_graph() -> nx.Graph:
    ...
    add_edge('Gate_A', 'Restroom_1')
    add_edge('Gate_A', 'Section_101')
    add_edge('Gate_A', 'Medical_1')

    add_edge('Gate_B', 'Concession_2')
    add_edge('Gate_B', 'Restroom_2')
    add_edge('Gate_B', 'Section_102')
    add_edge('Gate_B', 'Section_202')

    add_edge('Gate_C', 'Concession_3')
    add_edge('Gate_C', 'Restroom_3')
    add_edge('Gate_C', 'Section_103')
    add_edge('Gate_C', 'Medical_2')

    add_edge('Gate_D', 'Concession_4')
    add_edge('Gate_D', 'Restroom_4')
    add_edge('Gate_D', 'Section_104')
    add_edge('Gate_D', 'Section_203')

    # Connect concessions to sections and restrooms
    add_edge('Concession_1', 'Section_101')
    add_edge('Concession_1', 'Section_201')
    add_edge('Concession_2', 'Section_102')
    add_edge('Concession_2', 'Section_202')
    add_edge('Concession_3', 'Section_103')
    add_edge('Concession_3', 'Section_203')
    add_edge('Concession_4', 'Section_104')
    add_edge('Concession_4', 'Section_204')

    add_edge('Restroom_1', 'Section_101')
    add_edge('Restroom_1', 'Section_201')
    add_edge('Restroom_2', 'Section_102')
    add_edge('Restroom_2', 'Section_202')
    add_edge('Restroom_3', 'Section_103')
    add_edge('Restroom_3', 'Section_203')
    add_edge('Restroom_4', 'Section_104')
    add_edge('Restroom_4', 'Section_204')

    # Connect sections to each other (adjacent)
    add_edge('Section_101', 'Section_102')
    add_edge('Section_101', 'Section_103')
    add_edge('Section_102', 'Section_104')
    add_edge('Section_103', 'Section_104')
    add_edge('Section_201', 'Section_202')
    add_edge('Section_201', 'Section_203')
    add_edge('Section_202', 'Section_204')
    add_edge('Section_203', 'Section_204')

    # Connect medical to nearby sections
    add_edge('Medical_1', 'Section_101')
    add_edge('Medical_1', 'Section_201')
    add_edge('Medical_2', 'Section_103')
    add_edge('Medical_2', 'Section_203')
    ...
    kb_chunks = [
        {"id": "faq_1", "text": "Where are the restrooms? Restrooms are located near each gate: Restroom_1 near Gate A, Restroom_2 near Gate B, Restroom_3 near Gate C, Restroom_4 near Gate D."},
        {"id": "faq_2", "text": "Where can I buy food? Concessions are located near each gate: Concession_1 (Hot Dogs) near Gate A, Concession_2 (Beverages) near Gate B, Concession_3 (Snacks) near Gate C, Concession_4 (Merchandise) near Gate D."},
        {"id": "faq_3", "text": "Where is medical assistance? Medical tents are located near Gate A (Medical_1) and Gate C (Medical_2)."},
        {"id": "faq_4", "text": "What sections are available? Lower bowl sections 101-104 and upper bowl sections 201-204."},
        {"id": "map_info", "text": "The stadium has four gates: A (North), B (East), C (South), D (West). Each gate has nearby concessions, restrooms, and medical aid."}
    ]
