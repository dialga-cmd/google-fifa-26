...
# Configuration lives in src/config.py. Import it in a way that works both when
# this module is loaded as a package (``src.api``, e.g. ``uvicorn src.api:app``)
# and as a top-level module (``api``, e.g. the test suite prepends ``src`` to
# ``sys.path`` and does ``from api import ...``).
try:
    from .config import Config
except ImportError:  # pragma: no cover - fallback for top-level import
